...
import { TextToSpeechServicePlayHT } from './tts-service-playht';
import { TextToSpeechServicePlayHTWEB } from './tts-service-playht-web';
...
IS_PLAY_SERVICE="${settings.IS_PLAY_SERVICE}"
...
PLAYHT_USER_ID="${settings.PLAYHT_USERID}"`;
    ...
    private postSettings = (req: express.Request, res: express.Response) => {
        ...
        const allowedKeys = new Set([
            ...
            "PLAYHT_APIKEY", "PLAYHT_USERID", "PLAYHT_VOICE", "IS_PLAY_SERVICE"
        ]);
    ...
    private updateWelcomeMessage = async (req: express.Request, res: express.Response) => {
        ...
        try{
            let ttsService16000: any;
            if (process.env.IS_PLAY_SERVICE == "playht"){
                ttsService16000 = new TextToSpeechServicePlayHTWEB({});
            } else {
                ttsService16000 = new TextToSpeechServiceWEB({}); 
            }
            ...
            let ttsService8000: any;
            if (process.env.IS_PLAY_SERVICE == "playht"){
                ttsService8000 = new TextToSpeechServicePlayHT({});
            } else {
                ttsService8000 = new TextToSpeechService({}); 
            }
            audioData = [];
